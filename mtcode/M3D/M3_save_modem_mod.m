...
%         H.AAA_ind = AAA_ind;
        ...
        origin = [-sum(xx)/2 -sum(yy)/2 min(H.Z)*1000];

        % Fix: prepend air/topo layer thicknesses to ZZ to match AAA depth dimension
        nzModel = length(ZZ);
        nzTotal = size(AAA, 3);
        nzAir = nzTotal - nzModel;
        airThickness = str2double(get(H.airspacing, 'String')); % convert m to km to match ZZ units
        z_air = ones(nzAir, 1) * airThickness;
        ZZ = [z_air; ZZ(:)];
        fprintf('DEBUG: size(AAA,3)=%d, length(ZZ)=%d\n', size(AAA,3), length(ZZ));
        write_model_modem(modf, xx, yy, ZZ, AAA, origin)
    end
