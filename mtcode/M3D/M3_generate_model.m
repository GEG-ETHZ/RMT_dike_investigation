...
% Display model in M3DET window using parameters entered on screen.
    ...
    H.Z=z/1000; H.XX=sort([H.XX,xpn,xpp]); H.YY=sort([H.YY,ypn,ypp])'; %x, y and z defined as vectors
    ...
    m.dx = round(abs(diff(H.XX .* 1000))', 4);  % multiply first, then diff
    m.dy = round(abs(diff(H.YY .* 1000)), 4);
    m.dz = round(abs(diff(H.Z .* 1000))', 4);
