...
nzAir = 20; %DC: I am not sure how ModEM handles air layers. My understanding is that there are zero air layers
rotation=0;
...
for j = 1:nx
    status = fprintf(fid,'%4f ',x(j));
end
fprintf(fid, '\n');
for j = 1:ny
    status = fprintf(fid,'%4f ',y(j));
end
fprintf(fid, '\n');

for j = 1:nz
    status = fprintf(fid,'%4f ',z(j));
end
fprintf(fid, '\n');
for k = 1:nz
