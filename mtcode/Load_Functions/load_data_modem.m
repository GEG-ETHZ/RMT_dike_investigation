...
fid = fopen(name,'r');
nlines = 0; hc = 0;
data_fmt = '%f %s %f %f %f %f %f %s %f %f %f %f %f %f %f';
while 1
    ...
    if tline(1) == '#'
        % Comment line (e.g. "# written by..." or "# Period(s)...")
        continue
    elseif tline(1) == '>'
        % Six-line ModEM header block starting with ">"
        headerLines = cell(6,1);
        headerLines{1} = strtrim(tline(2:end));
        for k = 2:6
            hline = fgetl(fid);
            if hline == -1
                error('Unexpected end of file while reading ModEM header block')
            end
            headerLines{k} = strtrim(hline(2:end));
        end
        hc = hc+1;
        header{hc} = char(headerLines);
        dataType{hc} = strtrim(header{hc}(1,:));
        signstr{hc}  = header{hc}(2,:);
        if ~isempty(strfind(signstr{hc},'-'))
            isign(hc) = -1;
        else
            isign(hc) = +1;
        end
    ...
    elseif ~isempty(sscanf(line, '%f', 1))
        nlines = nlines+1;
        data = textscan(line, data_fmt);
        
        T(nlines) = [data{1}];
        site{nlines} = data{2}{1};
        loc(nlines,1) = [data{3}];
        loc(nlines,2) = [data{4}];
        loc(nlines,3) = [data{7}];
...
for i = 1:nlines
    ...
    ifreq = find(d.T == T(i), 1);
    isite = find(strcmp(site{i}, d.site), 1);
    icomp = find(strcmp(responses{i}, components), 1);
    
    if isempty(ifreq) || isempty(isite) || isempty(icomp)
        continue
    end
