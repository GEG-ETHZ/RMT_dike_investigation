function [x,y,z,rho] = plot_diagonal_section_RV(m,d)
...
% ---- publication-style dike highlight options (matches plot_model_3D_RV.m) ----
...
% helper: inject a narrow band of dike_color into the current colormap
% at the resistivity value corresponding to dike_ohmm, without touching
% the underlying data. Call this AFTER colormap+caxis are set for the
% axes you want to modify.
    function apply_dike_highlight()
        if ~highlight_dike
            return
        end
...
apply_dike_highlight(); % highlight the dike on the map slice too
...
apply_dike_highlight(); % recolor the dike bright green on the cross-section too
