...
for ifreq = 1:u.nskip:d.nf  % Loop over frequencies
   %%
    ...
    lat_ticks = linspace(min(d.loc(:,1)), max(d.loc(:,1)), 5);
    lon_ticks = linspace(min(d.loc(:,2)), max(d.loc(:,2)), 5);
    ...
    if iv
        fname = ['phase_tensor_iv_map_',u.phase_tensor_ellipse_fill,'_',num2str(ifreq,'%03.0f'),'_',num2str(d.T(ifreq)),'_s'];
        print_figure(['phase_tensor_',d.niter], fname); %Save figure (original format)
        outdir = ['phase_tensor_',d.niter];
        if ~exist(outdir,'dir'), mkdir(outdir); end
        print(gcf, '-dsvg', fullfile(outdir, [fname, '.svg']));
    else
        fname = ['phase_tensor_map_',u.phase_tensor_ellipse_fill,'_',num2str(ifreq,'%03.0f'),'_',num2str(d.T(ifreq)),'_s'];
        print_figure(['phase_tensor_',d.niter], fname); %Save figure (original format)
        outdir = ['phase_tensor_',d.niter];
        if ~exist(outdir,'dir'), mkdir(outdir); end
        print(gcf, '-dsvg', fullfile(outdir, [fname, '.svg']));
